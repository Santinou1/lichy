...
function ActualizarEstado({setHistorial, contenedor, actualizarEstado,estad,ubicacio}){
    ...
    useEffect(()=>{
        axios.get('http://192.168.0.131:5000/api/items/categorias').then((response)=>{
    ...
    useEffect(()=>{
        axios.post('http://192.168.0.131:5000/api/items/ubicaciones',{estado: estado}).then((response)=>{
            console.log(response.data);
    ...
    const onSubmit = (e) =>{
        ...
        axios.post('http://192.168.0.131:5000/api/contenedorEstado/',{contenedor,ubicacion,estado,fechaManual: fechaAEnviar}).then((response)=>{
