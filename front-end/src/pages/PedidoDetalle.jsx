...
function PedidoDetalle() {
  ...
  const cargarDatosPedido = async () => {
    try {
      ...
      const response = await axios.get(`http://192.168.0.131:5000/api/pedidos/${id}`);
      setPedido(response.data);
      
      // Cargar productos asociados al pedido
      const productosResponse = await axios.get(`http://192.168.0.131:5000/api/pedidos/${id}/productos`);
  ...
  const cargarProductosDisponibles = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.0.131:5000/api/contenedorProducto/disponibles');
  ...
  const eliminarProducto = async (idProductoPedido) => {
    ...
    try {
      // Esta llamada API necesitará implementarse en el backend
      await axios.delete(`http://192.168.0.131:5000/api/pedidos/${id}/productos/${idProductoPedido}`);
